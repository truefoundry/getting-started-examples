...
import uuid
import logging
from agent import create_workflow, SQLQueryResult, PlotResult
import logging
...
async def process_query(job_id: str, query: str):
    ...
    try:
        ...
        # Record the SQL execution event
        results_store[job_id]["events"].append({
            "event": "sql_execution",
            "content": f"Executed SQL query: {result['sql_result'].query}"
        })
        
        # Check for SQL errors
        if result['sql_result'].error:
            results_store[job_id]["status"] = "failed"
            results_store[job_id]["error"] = result['sql_result'].error
            results_store[job_id]["events"].append({
                "event": "workflow_error",
                "content": f"SQL Query Error: {result['sql_result'].error}"
            })
            return
        
        # Record the visualization event
        if 'plot_result' in result and result['plot_result']:
            ...
            # Update status and store plot result
            results_store[job_id]["status"] = "completed"
            results_store[job_id]["plot_result"] = plot_result.model_dump()
            results_store[job_id]["events"].append({
                "event": "visualization_complete",
                "content": f"Created {plot_result.plot_type} plot with {plot_result.x_col} and {plot_result.y_col or ''}"
            })
        else:
            results_store[job_id]["status"] = "failed"
            results_store[job_id]["error"] = "No plot result was generated"
            results_store[job_id]["events"].append({
                "event": "visualization_error",
                "content": "No plot result was generated"
            })
